...
2. **Template Selection (ОБЯЗАТЕЛЬНЫЙ ШАГ для документации):** Если распарсенный контент является проектной/продуктовой документацией (требования, спецификации, архитектура, инфраструктура, маркет-анализ, качество):
   - Определи тип документа по `reference/decision-matrix.md`.
   - Если контент не соответствует ни одному типу из матрицы — пропусти шаблон, используй свободную структуру как обычно.
   - Возьми `templates/base.md` + аддон(ы) из `templates/addons/`, соответствующие определённому типу (например, PRD → `addons/prd.md`; regulated-продукт → `addons/prd.md` + `addons/frd.md` + `addons/srd.md`).
   - Структурируй итоговый вывод согласно base + выбранные аддоны.
   - **Пустые поля шаблона = `[TBD]`. НИКОГДА не изобретай данные, метрики или имена.**
   - Этот шаг НЕ опционален для документационного контента.
3. **Разведка метаданных (`/kb`):** Выполни `kbcli --list-domains`, `kbcli --list-categories` и `kbcli --list-tags`. Максимально переиспользуй существующие сущности для нового YAML.
4. **Генерация YAML:** Сформируй frontmatter, строго соответствующий схеме `kb.articles`. Учитывай System Overrides, если они были.
5. **Поиск дублей (`/kb`):** Выполни `kbcli --tags <твой_тег>` или `kbcli --search "<заголовок>"`, чтобы найти совпадения.
6. **Deep Read (L2):** Если `/kb` вернул `vault_path`, нативно прочитай этот файл (`cat`/`grep`).
7. **Merge Logic (Семантическое слияние):**
...
8. **Фиксация (`/git_commit`):** Зафиксируй состояние Vault.
9. **Синхронизация L0/L1 (`/kbupd`):** Выполни `python3 /home/sm1ly/llm_instances/memtools/sync_l1_index.py --file <path_to_new_file>`.
